# Windows API 调用笔记：GetFileInformationByHandle

GetFileInformationByHandle 常出现在文件、目录、异步 I/O、完成端口和命名管道场景。这类接口覆盖文件系统对象、目录操作、文件属性、异步 I/O、完成端口和命名管道通信。记录时保留原始路径、最终对象、句柄访问掩码、共享模式、重解析点、OVERLAPPED 状态和管道端点身份。
...
```cpp
auto result = GetFileInformationByHandle(...);
```

```powershell
dumpbin /exports C:\Windows\System32\kernel32.dll | findstr /i GetFileInformationByHandle
```
