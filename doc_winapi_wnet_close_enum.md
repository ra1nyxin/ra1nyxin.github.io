# Windows API 调用笔记：WNetCloseEnum

WNetCloseEnum 多见于设备枚举、驱动、无线和系统能力查询场景，主要处理枚举设备、读取设备属性、处理通知、查询无线配置、检查驱动和系统能力。它们常见于资产采集、硬件诊断、终端管控、无线排查和安全代理。
...
```cpp
auto result = WNetCloseEnum(...);
```

```powershell
dumpbin /exports C:\Windows\System32\mpr.dll | findstr /i WNetCloseEnum
```
