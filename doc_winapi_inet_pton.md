# Windows API 调用笔记：inet_pton

inet_pton 常见于套接字建立、地址转换、连接控制和数据收发场景。判断时要把协议族、阻塞模式、目标地址和后续 I/O 放在同一条链路里看。
## 入口
...
```powershell
dumpbin /exports C:\Windows\System32\ws2_32.dll | findstr /i inet_pton
```
