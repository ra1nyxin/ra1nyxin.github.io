# Windows API 调用笔记：ldap_msgfree

ldap_msgfree 常见于 LDAP 连接建立、目录查询、属性遍历和绑定认证场景。分析时先分清它处理的是会话、搜索结果、属性值还是认证过程。
## 入口
...
```cpp
auto result = ldap_msgfree(...);
```

```powershell
dumpbin /exports C:\Windows\System32\wldap32.dll | findstr /i ldap_msgfree
```
