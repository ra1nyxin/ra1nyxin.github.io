# Windows API 调用笔记：K32QueryWorkingSet

K32QueryWorkingSet 在进程内存、性能计数和资源状态查询里很常见，覆盖的内容主要是读取进程内存占用、工作集、句柄数量、I/O 计数、时间片和缓解策略等运行状态。它适合性能排查、取证基线、异常进程分析和沙箱行为记录。
...
```cpp
auto result = K32QueryWorkingSet(...);
```

```powershell
dumpbin /exports C:\Windows\System32\psapi.dll | findstr /i K32QueryWorkingSet
```
