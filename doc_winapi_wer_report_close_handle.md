# Windows API 调用笔记：WerReportCloseHandle

WerReportCloseHandle 常出现在错误码、环境块、用户配置文件和版本信息场景，主要处理解释错误、处理用户配置文件、构造环境块、读取版本资源或确认应用身份。它们经常是排查链路里的辅助证据，但会直接影响最终结论。
...
```cpp
auto result = WerReportCloseHandle(...);
```

```powershell
dumpbin /exports C:\Windows\System32\wer.dll | findstr /i WerReportCloseHandle
```
