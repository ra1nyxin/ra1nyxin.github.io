# Windows API 调用笔记：GetAclInformation

GetAclInformation 放到注册表、ACL、SID 和安全描述符处理场景里看并不罕见。这类接口多用于配置读取、权限判断、持久化排查、策略核查和对象安全边界分析。注册表与安全描述符都要保留原始输入和解析结果，单独的显示名或字符串路径不足以支撑结论。
...
```cpp
auto result = GetAclInformation(...);
```

```powershell
dumpbin /exports C:\Windows\System32\advapi32.dll | findstr /i GetAclInformation
```
