...
AmsiUninitialize 放到AMSI 内容扫描和反恶意软件集成场景里看并不罕见。AMSI 接口用于把脚本、宏、内存缓冲区或应用内容交给反恶意软件提供程序评估。常见落点包括 PowerShell、脚本宿主、Office、浏览器组件和自带脚本引擎的软件。记录时保留应用名、会话、内容来源、内容长度、扫描结果和提供程序状态。
