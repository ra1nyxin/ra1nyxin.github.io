# Windows API 调用笔记：CM::Get::Device::Interface::List::SizeW

CM::Get::Device::Interface::List::SizeW 常见于 设备枚举、驱动、无线和系统能力查询排查。实际使用时，它们通常围绕枚举设备、读取设备属性、处理通知、查询无线配置、检查驱动和系统能力。它们常见于资产采集、硬件诊断、终端管控、无线排查和安全代理。
...
```cpp
// COM method: CM::Get::Device::Interface::List::SizeW(...)
```
