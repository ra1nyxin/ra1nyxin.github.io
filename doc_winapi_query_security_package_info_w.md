# Windows API 调用笔记：QuerySecurityPackageInfoW

QuerySecurityPackageInfoW 经常落在SSPI、Schannel 和认证上下文这类位置。SSPI 接口处理认证包、凭据句柄、安全上下文、消息保护和模拟。常见落点包括 Kerberos、NTLM、Negotiate、Schannel、RPC、HTTP 认证和自定义客户端服务端协议里。记录时要明确认证包、目标 SPN、上下文属性、缓冲区方向和状态码。
...
```cpp
auto result = QuerySecurityPackageInfoW(...);
```

```powershell
dumpbin /exports C:\Windows\System32\secur32.dll | findstr /i QuerySecurityPackageInfoW
```
