# Windows API 调用笔记：DnsFlushResolverCacheEntry_W

DnsFlushResolverCacheEntry_W 常见于名称解析、缓存管理和 DNS 记录读写场景。排查时先分清它处理的是查询结果、缓存条目、上下文句柄还是记录集合。
## 入口
...
```cpp
auto result = DnsFlushResolverCacheEntry_W(...);
```

```powershell
dumpbin /exports C:\Windows\System32\dnsapi.dll | findstr /i DnsFlushResolverCacheEntry_W
```
