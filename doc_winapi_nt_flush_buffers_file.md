...
NtFlushBuffersFile 我会放在 NT 文件、目录、EA、重命名和链接操作复核 时查。先做最小调用，把返回值、错误码和调用上下文写清楚，再放回具体样本或现场里判断。
...
```cpp
auto result = NtFlushBuffersFile(...);
```

```powershell
dumpbin /exports C:\Windows\System32\ntdll.dll | findstr /i NtFlushBuffersFile
```
