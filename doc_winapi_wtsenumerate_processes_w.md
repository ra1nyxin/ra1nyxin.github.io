# Windows API 调用笔记：WTSEnumerateProcessesW

WTSEnumerateProcessesW 放到服务控制、目录服务、SAM、NetAPI 和会话管理场景里看并不罕见。这类调用常用来服务管理、账号枚举、域信息查询、会话读取和远程管理。判断时要明确目标主机、调用身份、访问掩码、分页状态和返回结构级别。
...
```cpp
auto result = WTSEnumerateProcessesW(...);
```

```powershell
dumpbin /exports C:\Windows\System32\wtsapi32.dll | findstr /i WTSEnumerateProcessesW
```
