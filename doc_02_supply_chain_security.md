...
软件供应链安全关注的是代码从编写、依赖引入、构建、发布到运行的完整链路。需要重点确认这些问题：谁能改代码、谁能改构建、谁能发布、谁能替换制品，以及依赖漏洞是否会影响真实运行路径。
...
依赖治理重点是风险分层，简单把所有包升到最新会带来兼容性和回归成本。
