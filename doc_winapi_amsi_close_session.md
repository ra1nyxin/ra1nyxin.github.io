...
AmsiCloseSession 在AMSI 内容扫描和反恶意软件集成里出现得很多。AMSI 接口用于把脚本、宏、内存缓冲区或应用内容交给反恶意软件提供程序评估。常见落点包括 PowerShell、脚本宿主、Office、浏览器组件和自带脚本引擎的软件。记录时保留应用名、会话、内容来源、内容长度、扫描结果和提供程序状态。
...
整理证据时，把扫描内容来源、宿主进程、脚本路径、用户、AMSI Provider 和结果放在一起。若出现扫描失败、Provider 缺失或会话异常，需要关联事件日志和安全产品状态。
