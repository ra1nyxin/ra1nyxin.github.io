# Windows API 调用笔记：CM_Get_Parent

CM_Get_Parent 常见于设备树遍历、驱动排错、硬件属性读取和终端资产采集场景。判断时要先分清它处理的是节点关系、接口信息还是设备属性，再看调用前后的枚举状态是否一致。
## 入口
...
```cpp
auto result = CM_Get_Parent(...);
```

```powershell
dumpbin /exports C:\Windows\System32\cfgmgr32.dll | findstr /i CM_Get_Parent
```
