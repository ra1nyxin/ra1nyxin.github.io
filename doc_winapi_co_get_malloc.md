...
CoGetMalloc 我会放在 COM+ Catalog、组件注册和 COM 安全边界复核 时查。先做最小调用，把返回值、错误码和调用上下文写清楚，再放回具体样本或现场里判断。
...
```cpp
auto result = CoGetMalloc(...);
```

```powershell
dumpbin /exports C:\Windows\System32\comsvcs.dll | findstr /i CoGetMalloc
```
