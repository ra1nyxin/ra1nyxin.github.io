# Windows API 调用笔记：ConnectNamedPipe

ConnectNamedPipe 常出现在文件、目录、异步 I/O、完成端口和命名管道场景。这类接口覆盖文件系统对象、目录操作、文件属性、异步 I/O、完成端口和命名管道通信。记录时保留原始路径、最终对象、句柄访问掩码、共享模式、重解析点、OVERLAPPED 状态和管道端点身份。
...
```cpp
auto result = ConnectNamedPipe(...);
```

```powershell
dumpbin /exports C:\Windows\System32\kernel32.dll | findstr /i ConnectNamedPipe
```
...
复查时，把路径、句柄来源、对象 ID、调用身份、I/O 大小和完成结果放在一起。管道通信还要关联客户端 PID、服务端 PID 和连接时间。
